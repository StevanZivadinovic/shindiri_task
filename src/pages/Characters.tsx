...
const Characters: React.FC = () => {
  const { characters, isLoading, isError, fetchNextPage, hasNextPage } = useCharacters();
  ...
  return (
    <div className="container mx-auto p-4 relative">
      ...
      {isError && <p className="text-red-500 text-center">Error loading characters.</p>}
      {data ? (<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {data?.results && data?.results?.map((character:any) => (
      ...
      {isLoading && <p className="text-center text-lg mt-4">Loading more characters...</p>}
    </div>
  );
};
